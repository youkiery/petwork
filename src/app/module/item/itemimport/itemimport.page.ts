...
@Component({
  ...
  templateUrl: './itemimport.page.html',
  styleUrls: ['./itemimport.page.scss'],
})
export class ItemimportPage implements OnInit {
  public name = 'Chưa chọn file Excel Bệnh viện'
  public name2 = 'Chưa chọn file Excel Kho'
  ...
  public file() {
    ...
    else this.name = 'Chưa chọn file Excel Bệnh viện'
  }

  public file2() {
    let file = this.pwaphoto2
    const fileList: FileList = file.nativeElement.files

    if (fileList.length) this.name2 = fileList[0].name
    else this.name2 = 'Chưa chọn file Excel Kho'
  }

  public async uploadfile() {
    const fileList: FileList = this.pwaphoto.nativeElement.files;
    const fileList2: FileList = this.pwaphoto2.nativeElement.files;

    let body = new FormData();
    if (!fileList[0] || !fileList2[0]) this.rest.notify('Chưa chọn file excel')
    else {
