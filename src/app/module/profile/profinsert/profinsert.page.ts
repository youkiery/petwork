...
export class ProfinsertPage {
  ...
  ionViewWillEnter() {
    ...
    else if (!this.rest.profile.sampletype.length) this.init()
  }

  public async init() {
    this.rest.checkpost('profile', 'initsample', {}).then(resp => {
      this.rest.defreeze()
      this.rest.profile.sampletype = resp.list
      this.rest.profile.sampletype = resp.target
      this.rest.back()
    }, () => {
      this.rest.defreeze()
    })
