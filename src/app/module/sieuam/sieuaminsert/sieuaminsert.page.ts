...
@Component({
  selector: 'app-sieuaminsert',
  templateUrl: './sieuaminsert.page.html',
  styleUrls: ['./sieuaminsert.page.scss'],
})
export class SieuaminsertPage {
  public count = 0
  public max = 2048
  ...
  ionViewWillEnter() {
    if (!this.rest.action.length) this.rest.navCtrl.navigateBack('/sieuam')
  }
  ...
  public async insert( ) {
    ...
    this.rest.checkpost('sieuam', 'insert', this.rest.temp).then(resp => {
      this.rest.defreeze()
      if (this.rest.temp.xrayid) this.rest.sieuam.s = '2'
  ...
  public async update( ) {
    ...
    await this.rest.freeze('Đang tải dữ liệu...')
    await this.uploadAllImage()
    this.rest.checkpost('sieuam', 'update', this.rest.temp).then(resp => {
      this.rest.defreeze()
      this.rest.sieuam.init = true
      this.rest.sieuam.list = resp.list
      this.rest.his.init = false
