...
@Component({
  selector: 'app-xquanginsert',
  templateUrl: './xquanginsert.page.html',
  styleUrls: ['./xquanginsert.page.scss'],
})
export class XquanginsertPage {
  public count = 0
  public max = 2048
  ...
  ionViewWillEnter() {
    if (!this.rest.action.length) this.rest.navCtrl.navigateBack('/xquang')
  }
  ...
  public async insert( ) {
    ...
    this.rest.checkpost('xquang', 'insert', this.rest.temp).then(resp => {
      this.rest.defreeze()
      if (!this.rest.temp.his) {
        if (this.rest.temp.xrayid) this.rest.xquang.s = '2'
  ...
  public async update( ) {
    ...
    await this.rest.freeze('Đang tải dữ liệu...')
    await this.uploadAllImage()
    this.rest.checkpost('xquang', 'update', this.rest.temp).then(resp => {
      this.rest.defreeze()
      this.rest.xquang.init = true
      this.rest.xquang.list = resp.list
      this.rest.his.init = false
