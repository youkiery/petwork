...
@Component({
  ...
  templateUrl: './hotelinsert.page.html',
  styleUrls: ['./hotelinsert.page.scss'],
})
export class HotelinsertPage {
  public count = 0
  public max = 1024
  public init = false
  ...
  ionViewWillEnter() {
    ...
    if (!this.init && !this.rest.temp.id) {
      this.init = true
      this.suggest()
    }
  }
  ...
  public async insert( ) {
    if (!this.rest.temp.image.length) {
      this.rest.notify('Xin hãy chọn ảnh')
      return 0
    }
    await this.rest.freeze('Đang tải dữ liệu...')
    await this.uploadAllImage()
    this.rest.checkpost('hotel', 'insert', this.rest.temp).then(resp => {
      this.rest.defreeze()
  ...
  public async update( ) {
    ...
    await this.rest.freeze('Đang tải dữ liệu...')
    await this.uploadAllImage()
    this.rest.checkpost('hotel', 'update', this.rest.temp).then(resp => {
      this.rest.defreeze()
      this.rest.hotel.list = resp.list
      this.rest.back()
    }, () => {
